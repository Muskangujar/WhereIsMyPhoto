...
def detect_faces(image_array):
    ...
    return face_locations, face_encodings, rgb_image
    ...
    return [
        {
            ...
            "image_url": "",
            "title": "⚠️ DEMO — Public Instagram Post",
        ...
        },
        {
            ...
            "image_url": "",
            "title": "⚠️ DEMO — Public Tweet with Photo",
            "domain": "twitter.com",
            "match_type": "similar",
            "similarity": 87,
        },
...
if uploaded_file is not None:
    ...
    with st.spinner("🔍 Detecting faces..."):
        face_locations, face_encodings, rgb_image = detect_faces(image_array)
    ...
    # --- Face(s) found ---
    st.success(f"✅ {len(face_locations)} face{'s' if len(face_locations) > 1 else ''} detected")
    ...
    with col_q3:
        status = "✅ Good" if not quality["is_blurry"] and not quality["is_low_res"] else "⚠️ Low"
        st.metric("Quality", status)

    if quality["is_blurry"]:
        st.warning("⚠️ Face is blurry — results may be less accurate.")
    if quality["is_low_res"]:
        st.warning(
            f"⚠️ Face is only {quality['width']}×{quality['height']}px "
    ...
    if st.button("🔍 Search the web for this face", type="primary", use_container_width=True):
        ...
        if is_demo:
            st.warning("⚠️ **These are DEMO results, not real matches.** Set SERPAPI_KEY for real searches.")
        ...
        st.download_button(
            label="⬇️ Download Pipeline Output (JSON)",
            data=output_json,
